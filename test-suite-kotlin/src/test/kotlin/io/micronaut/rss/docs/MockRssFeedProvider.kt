...
class MockRssFeedProvider : RssFeedProvider {
    val rssChannel: RssChannel = RssChannel.builder("Liftoff News", "http://liftoff.msfc.nasa.gov/", "Liftoff to Space Exploration.")
    ...
    @SingleResult
    override fun fetch(): Publisher<RssChannel> = Publishers.just(rssChannel)

    @SingleResult
    override fun fetchById(id: Serializable?): Publisher<RssChannel> {
        if (id != null && id == "1") {
