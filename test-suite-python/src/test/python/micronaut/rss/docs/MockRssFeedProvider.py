...
from micronaut.rss import RssChannel, RssFeedProvider, RssItem
from micronaut.rss.language import RssLanguage
from org.reactivestreams import Publisher
# end::imports[]


# See https://cyber.harvard.edu/rss/examples/rss2sample.xml
@Requires(property="spec.name", value="MockRssFeedProviderTest")
# tag::class[]
@Singleton
class MockRssFeedProvider(RssFeedProvider):

    def __init__(self):
        self.rss_channel = (RssChannel.builder("Liftoff News", "http://liftoff.msfc.nasa.gov/", "Liftoff to Space Exploration.")
            ...
            .item(RssItem.builder()
                ...
                .pubDate(ZonedDateTime.of(LocalDateTime.of(2003, 5, 20, 8, 56, 2), ZoneId.of("GMT")))
                .guid("http://liftoff.msfc.nasa.gov/2003/05/20.html#item570")
                .build())
            .build())

    @SingleResult
    def fetch(self) -> Publisher[RssChannel]:
        return Publishers.just(self.rss_channel)

    @SingleResult
    def fetchById(self, id: Serializable | None) -> Publisher[RssChannel]:
